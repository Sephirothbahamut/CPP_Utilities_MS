#pragma once
//Note: this module is in the raw namespace because the moment you start using it, there's no reason to hide directx and win32 stuff anymore. You'll need to use them to write with direct2d.
//Abstracting the whole 2d drawing system is WAY outside the scope of this library. Use SFML for that.
	...
	class render_target : public details::base<render_target>
		...
		public:
			...
			render_target(utils::MS::window::base& base, create_info create_info) :
				...
				d2d_hwnd_rt{d2d::hwnd_render_target::create(create_info.d2d_factory, d2d::hwnd_render_target::create_info
					{
					.hwnd{get_base().get_handle()}
					})},
	...
	class swap_chain : public details::base<swap_chain>
		...
		private:
			...
			void on_resize(utils::math::vec2u size) noexcept
				...
				d2d_bitmap_target.com_ptr = nullptr;
			...
			d2d::bitmap create_bitmap_target() const noexcept
				{
				d2d::bitmap ret
					{
					d2d_device_context,
					d2d::bitmap::create_info
						{
						.dxgi_format{DXGI_FORMAT_B8G8R8A8_UNORM},
						.alpha_mode {D2D1_ALPHA_MODE_PREMULTIPLIED},
						.options    {D2D1_BITMAP_OPTIONS_TARGET | D2D1_BITMAP_OPTIONS_CANNOT_DRAW}
						},
					dxgi_swapchain
					};
				return ret;
