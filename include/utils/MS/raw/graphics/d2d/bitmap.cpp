...
namespace utils::MS::raw::graphics::d2d
	...
	bitmap::bitmap(const d2d::context& d2d_context, const create_info& create_info, utils::math::vec2s size)
		...
		winrt::check_hresult(d2d_context->CreateBitmap(D2D1_SIZE_U{static_cast<uint32_t>(size.x()), static_cast<uint32_t>(size.y())},
			nullptr,
			0,//Pitch is only necessary when there's a source data assigned
	...
	bitmap::bitmap(const d2d::context& d2d_context, const create_info& create_info) : bitmap{d2d_context, create_info, d2d::cast(d2d_context->GetPixelSize())} {}
	
	bitmap::bitmap(const d2d::context& d2d_context, utils::math::vec2s size) : bitmap
		{
		d2d_context,
		create_info
			{
	...
	bitmap::bitmap(const d2d::context& d2d_context) : bitmap{d2d_context, d2d::cast(d2d_context->GetPixelSize())}{}
	...
	utils::matrix<utils::graphics::colour::rgba_f> bitmap::to_cpu_matrix(d2d::context& d2d_context) const
		{
		if (com_ptr->GetPixelFormat().format != DXGI_FORMAT_R32G32B32A32_FLOAT)
			{
			throw std::logic_error{"Operation only supported for DXGI_FORMAT_R32G32B32A32_FLOAT bitmaps"};
			}

		const auto pixel_size{com_ptr->GetPixelSize()};
		const auto resolution{d2d::cast(pixel_size)};
		...
		D2D1_RECT_U copy_src_rect{.left{0}, .top{0}, .right{pixel_size.width}, .bottom{pixel_size.height}};
		cpu_bitmap->CopyFromBitmap(&copy_dest_point, com_ptr.get(), &copy_src_rect);
		...
		utils::matrix<utils::graphics::colour::rgba_f> ret{static_cast<utils::math::vec2s>(resolution)};
