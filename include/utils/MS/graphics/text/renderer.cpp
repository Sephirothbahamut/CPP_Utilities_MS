...
namespace utils::MS::graphics::text
	...
	struct renderer::implementation : utils::oop::non_copyable, utils::oop::non_movable
		...
		umrg::d2d ::factory d2d_factory{nullptr};
		umrg::dw  ::factory dw_factory {nullptr};
		umrg::d3d ::device  d3d_device {nullptr};
		umrg::dxgi::device  dxgi_device{nullptr};
		umrg::d2d ::device  d2d_device {nullptr};
		umrg::d2d ::context d2d_context{nullptr};
		umrg::d2d ::bitmap  gpu_bitmap {nullptr};
		...
		void clear(const utils::graphics::colour::rgba_f& colour = utils::graphics::colour::rgba_f{0.f})
			...
			utils::MS::raw::throw_if_failed(d2d_context->EndDraw());
			}
		...
		void create_bitmap(const utils::math::vec2s& resolution)
			...
			gpu_bitmap = umrg::d2d::bitmap{d2d_context, umrg::d2d::bitmap::create_info
				{
			...
			d2d_context->SetTarget(gpu_bitmap);
			}
		...
		void draw_text(const format& format, const std::string& string, const utils::math::rect<float> region)
			...
			umrg::d2d::brush brush{d2d_context, format.colour};

			umrg::dw::text_format dw_format{dw_factory, format};
			
			umrg::dw::text_layout dw_layout{dw_factory, dw_format, string, region.size()};
			...
			d2d_context->BeginDraw();
			d2d_context->DrawTextLayout(umrg::d2d::cast(region.ul()), dw_layout, brush, D2D1_DRAW_TEXT_OPTIONS_ENABLE_COLOR_FONT | D2D1_DRAW_TEXT_OPTIONS_CLIP);
		...
		utils::matrix<utils::graphics::colour::rgba_f> get_image()
			{
			umrg::d2d::bitmap cpu_bitmap{d2d_context, umrg::d2d::bitmap::create_info
				{
				.resolution{resolution},
				.dxgi_format{DXGI_FORMAT_R32G32B32A32_FLOAT},//DXGI_FORMAT_B8G8R8A8_UNORM
				.alpha_mode {D2D1_ALPHA_MODE_PREMULTIPLIED},
			...
			cpu_bitmap->CopyFromBitmap(&copy_dest_point, gpu_bitmap, &copy_src_rect);
