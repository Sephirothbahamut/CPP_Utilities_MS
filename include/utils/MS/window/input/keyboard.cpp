...
namespace utils::MS::window::input
	{
	keyboard::keyboard(window::base& base, const keyboard::create_info& create_info) : module{base}, input_manager{create_info.input_manager}
		{
		//mouse
		...
		if (!RegisterRawInputDevices(&rid_keyboard, 1, sizeof(rid_keyboard))) { throw raw::last_error("failed to register raw input devices"); }
		}
	
	procedure_result keyboard::procedure(UINT msg, WPARAM wparam, LPARAM lparam)
		{
		switch (msg)
			{
	...
	void keyboard::default_process_digital(const input_keyboard::button_id& id, bool new_state) noexcept
		{
	...
	void ClearKeyboardBuffer(uint16_t vkCode)
		{
