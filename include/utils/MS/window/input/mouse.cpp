...
namespace utils::MS::window::input
	{
	mouse::mouse(window::base& base, const create_info& create_info) : module{base}, input_manager{create_info.input_manager}
		{
		//mouse
		...
		if (!RegisterRawInputDevices(&rid_mouse, 1, sizeof(rid_mouse))) { throw raw::last_error("failed to register raw input devices"); }
		}
	void mouse::capture() const noexcept { SetCapture(get_base().get_handle()); }
	void mouse::release() const noexcept { ReleaseCapture(); }
	void mouse::prepare_mouseleave() const noexcept
		{
		TRACKMOUSEEVENT tme
			{
	...
	utils::math::vec2l mouse::eval_vec2(LPARAM lparam) const noexcept
		{
		return {GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam)};
		}
	utils::input_system::device::mouse::button_id mouse::eval_extra_button(WPARAM wparam) const noexcept
		{
	...
	procedure_result mouse::procedure(UINT msg, WPARAM wparam, LPARAM lparam)
		{
		switch (msg)
			{
			case WM_MOUSEMOVE  : prepare_mouseleave(); default_process_analog (utils::math::vec2f{eval_vec2(lparam)}); return procedure_result::next(0);
	...
	void mouse::default_process_digital(const input_mouse::button_id& id, bool new_state) noexcept
		{
		default_mouse.digital.change(input_manager, id, new_state);
		}
	void mouse::default_process_analog(utils::math::vec2f new_state) noexcept
		{
	...
	bool mouse::wm_input(WPARAM wparam, LPARAM lparam)
		{
		UINT dataSize;
		...
		if (GetRawInputData(reinterpret_cast<HRAWINPUT>(lparam), RID_INPUT, rawdata.data(), &dataSize, sizeof(RAWINPUTHEADER)) == dataSize)
			...
			const auto device_handle{reinterpret_cast<uintptr_t>(raw->header.hDevice)};

			const auto result{mice.emplace(std::piecewise_construct, std::make_tuple(device_handle), std::make_tuple())};
			...
			if ((rawmouse.usFlags & MOUSE_MOVE_ABSOLUTE) == MOUSE_MOVE_ABSOLUTE)
				...
				const auto x{static_cast<long>((float(rawmouse.lLastX) / 65535.0f) * float(width ))};
				const auto y{static_cast<long>((float(rawmouse.lLastY) / 65535.0f) * float(height))};

				//Move to
				mouse.analog[input_mouse::axis_id::x].change(input_manager, static_cast<float>(x));
				mouse.analog[input_mouse::axis_id::y].change(input_manager, static_cast<float>(y));
				}
			else if (rawmouse.lLastX != 0 || rawmouse.lLastY != 0)
				{
				const long x{rawmouse.lLastX};
				const long y{rawmouse.lLastY};
