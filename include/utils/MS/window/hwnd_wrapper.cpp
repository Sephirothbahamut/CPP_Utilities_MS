...
#pragma region rects
	...
	void hwnd_wrapper::set_window_rect(rect_t rect) noexcept
		{
		const RECT rectangle{MS::cast(add_shadow_size_to(rect))};
	...
	rect_t hwnd_wrapper::add_shadow_size_to(rect_t rect) const noexcept
		...
		return
			{
			rect.ll() + shadow_thickness.ll(),
			rect.up() + shadow_thickness.up(),
			rect.rr() + shadow_thickness.rr(),
			rect.dw() + shadow_thickness.dw(),
			};
		}
