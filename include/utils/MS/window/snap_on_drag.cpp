...
namespace utils::MS::window
		...
		switch (msg)
			...
			case WM_MOVING:
				if (true)
					...
					const rect_t attempted_result{get_base().remove_shadow_size_from(MS::cast(attempted_result_win32))};
					...
					if (const auto& result_new_opt{evaluate_move()})
						...
						attempted_result_win32 = MS::cast(get_base().add_shadow_size_to(result_new));
	...
	std::optional<rect_t> snap_on_drag::evaluate_move()
		...
		if (snap.ll()) { ret.proxy_x() = closest.ll(); }
		if (snap.up()) { ret.proxy_y() = closest.up(); }
		if (snap.rr()) { ret.proxy_x() = closest.rr() - target.width (); }
		if (snap.dw()) { ret.proxy_y() = closest.dw() - target.height(); }
